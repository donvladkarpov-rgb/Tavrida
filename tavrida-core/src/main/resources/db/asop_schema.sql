...
COMMENT ON TABLE ASOP_ROLES IS 'Роли доступа в системе: администраторы, кассиры, водители, контролеры, ревизоры.';
INSERT INTO ASOP_ROLES (ROLE_ID, ROLE_NAME) VALUES
                                                (gen_random_uuid(), 'Администратор'), (gen_random_uuid(), 'Администратор перевозчика'), (gen_random_uuid(), 'Кассир'),
                                                (gen_random_uuid(), 'Водитель'), (gen_random_uuid(), 'Диспетчер'), (gen_random_uuid(), 'Контролер'), (gen_random_uuid(), 'Ревизор');
...
COMMENT ON TABLE ASOP_CARD_TYPES IS 'Типы провозных носителей: физические чипы (MIFARE), банковские карты, QR-коды, служебные.';
INSERT INTO ASOP_CARD_TYPES (CARD_TYPE_ID, CARD_TYPE_NAME) VALUES
                                                               (gen_random_uuid(), 'MIFARE DESFire EV3'), (gen_random_uuid(), 'Bank Card (EMV)'), (gen_random_uuid(), 'QR/Virtual'), (gen_random_uuid(), 'Employee');
...
COMMENT ON TABLE ASOP_TARIFF_TYPES IS 'Виды тарифных продуктов: пакет поездок, безлимит по времени, электронный кошелёк.';
INSERT INTO ASOP_TARIFF_TYPES (TARIFF_TYPE_ID, CODE, NAME, DESCRIPTION) VALUES
                                                                            (gen_random_uuid(), 'RIDES_PACKAGE', 'Пакет поездок до даты', 'Фиксированное количество поездок, ограниченное сроком.'),
                                                                            (gen_random_uuid(), 'UNLIMITED_TILL_DATE', 'Безлимит до даты', 'Безлимитные поездки в заданном периоде (день/месяц/год).'),
...
COMMENT ON TABLE ASOP_EVENT_TYPES IS 'Классификатор системных событий для аудита: создание сущностей, смена смен, валидация, ошибки.';
INSERT INTO ASOP_EVENT_TYPES (EVENT_TYPE, EVENT_TYPE_NAME) VALUES
...
COMMENT ON TABLE ASOP_TRANSACTION_TYPES IS 'Типы финансовых операций: пополнение, списание, возврат, холдирование.';
INSERT INTO ASOP_TRANSACTION_TYPES (TRANSACTION_TYPE_ID, TRANSACTION_TYPE_NAME) VALUES (gen_random_uuid(), 'Пополнение'), (gen_random_uuid(), 'Списание');
...
COMMENT ON TABLE ASOP_TRANSACTION_RESULTS IS 'Статусы завершения финансовых операций: успех, ожидание, технические/бизнес-ошибки.';
INSERT INTO ASOP_TRANSACTION_RESULTS (TRANSACTION_RESULT_ID, TRANSACTION_RESULT_NAME) VALUES
                                                                                          (gen_random_uuid(), 'В обработке / Ожидание'), (gen_random_uuid(), 'Успех'),
                                                                                          (gen_random_uuid(), 'Ошибка - карта не читается'), (gen_random_uuid(), 'Ошибка - недостаточно средств'),
                                                                                          (gen_random_uuid(), 'Ошибка - карта заблокирована'), (gen_random_uuid(), 'Ошибка - таймаут эквайринга');
...
COMMENT ON TABLE ASOP_ROUTE_TYPES IS 'Классификация маршрутов: основной маршрут, альтернативный путь, короткий рейс.';
INSERT INTO ASOP_ROUTE_TYPES (ROUTE_TYPE_ID, ROUTE_TYPE_NAME) VALUES (gen_random_uuid(), 'Маршрут'), (gen_random_uuid(), 'Путь');
...
COMMENT ON TABLE ASOP_CARRIERS IS 'Транспортные компании-перевозчики, эксплуатирующие транспорт и маршруты.';
...
COMMENT ON TABLE ASOP_USERS IS 'Участники системы: пассажиры (привязка по СНИЛС) и сотрудники перевозчиков. СНИЛС связывает учётную запись с реестром Минтруда.';
COMMENT ON COLUMN ASOP_USERS.SNILS IS 'Уникальный идентификатор гражданина в реестре Минтруда. Используется для поиска/создания профиля при оформлении льгот.';
COMMENT ON COLUMN ASOP_USERS.PHONE IS 'Контактный номер для уведомлений и верификации из реестра льгот.';
...
COMMENT ON TABLE ASOP_VEHICLES IS 'Парк транспортных средств (автобусы, трамваи, троллейбусы), закреплённый за перевозчиками.';
COMMENT ON COLUMN ASOP_VEHICLES.VEHICLE_NUMBER IS 'Государственный или внутренний бортовой номер.';
...
COMMENT ON TABLE ASOP_TERMINALS IS 'Бортовые валидаторы и кассовые устройства. Устанавливаются на транспорт или работают в кассах.';
COMMENT ON COLUMN ASOP_TERMINALS.BENEFITS_SYNC_TOKEN IS 'Контрольная сумма последнего пакета льгот. Используется для инкрементальной доставки обновлений на устройство.';
...
COMMENT ON TABLE ASOP_ROUTES IS 'Маршруты движения общественного транспорта. Определяют геометрию, расписание и правила применения социальных льгот.';
COMMENT ON COLUMN ASOP_ROUTES.BENEFIT_POLICY IS 'Правило применения субсидий на маршруте: ALL (все льготы работают), ALLOWLIST (только явно разрешённые в ASOP_ROUTE_BENEFITS), NONE (льготы отключены).';
COMMENT ON COLUMN ASOP_ROUTES.ROUTE_START_DATE IS 'Дата запуска маршрута в эксплуатацию.';
COMMENT ON COLUMN ASOP_ROUTES.ROUTE_END_DATE IS 'Дата вывода маршрута из расписания. NULL означает постоянное действие.';
...
COMMENT ON TABLE ASOP_FARE_ZONES IS 'Тарифные зоны для расчёта стоимости проезда. Определяются географическими полигонами.';
COMMENT ON COLUMN ASOP_FARE_ZONES.ZONE_POLYGON IS 'Граница зоны в координатах WGS84. Используется для определения принадлежности остановки к тарифному поясу.';
...
COMMENT ON TABLE ASOP_TRANSPORT_STOPS IS 'Остановочные пункты с геозонами посадки/высадки.';
COMMENT ON COLUMN ASOP_TRANSPORT_STOPS.ZONE_POLYGON IS 'Локальная геозона остановки. Определяет точку фактической валидации билета при проходе через турникет или валидатор.';
...
COMMENT ON TABLE ASOP_ROUTE_TRANSPORT_STOPS IS 'Порядок следования остановок на конкретном маршруте. SERIAL_NUMBER задаёт последовательность.';
...
COMMENT ON TABLE ASOP_TRANSPORT_STOP_ZONES IS 'Связь остановок с несколькими тарифными зонами (для сложных перекрёстков или кольцевых маршрутов).';
...
COMMENT ON TABLE ASOP_TARIFF_RATES IS 'Актуальные цены на тарифные продукты. Поддерживает кастомизацию по маршрутам, зонам и перевозчикам.';
COMMENT ON COLUMN ASOP_TARIFF_RATES.ROUTE_ID IS 'Привязка цены к конкретному маршруту. NULL означает цену для всех маршрутов.';
COMMENT ON COLUMN ASOP_TARIFF_RATES.CARRIER_ID IS 'Привязка к перевозчику. NULL означает общесистемную цену.';
COMMENT ON COLUMN ASOP_TARIFF_RATES.ZONE_ID IS 'Привязка к тарифной зоне. NULL означает универсальную цену.';
...
COMMENT ON TABLE ASOP_CARRIER_ZONES IS 'Базовая тарифная сетка перевозчика по зонам. Используется как резерв при отсутствии явной ставки в ASOP_TARIFF_RATES.';
...
COMMENT ON TABLE ASOP_CARDS IS 'Провозные носители. Изначально анонимны. Привязка к СНИЛС выполняется кассиром/админом для активации льгот.';
COMMENT ON COLUMN ASOP_CARDS.USER_ID IS 'Владелец карты. NULL до момента регистрации в кассе или личном кабинете.';
COMMENT ON COLUMN ASOP_CARDS.REGISTERED_AT IS 'Дата привязки карты к профилю льготника.';
COMMENT ON COLUMN ASOP_CARDS.REGISTERED_BY_USER_ID IS 'Сотрудник, выполнивший привязку. Фиксируется для аудита и отчётности Минтруда.';
...
COMMENT ON TABLE ASOP_CARD_MIFARES IS 'Технические параметры NFC-чипов MIFARE. UID используется для быстрой идентификации в валидаторе.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.UID IS 'Аппаратный серийный номер чипа.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.ATQA IS 'Ответ чипа на запрос SELECT. Определяет тип и поколение.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.SAK IS 'Код подтверждения выбора чипа.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.PROTOCOL_VERSION IS 'Версия криптографического протокола чипа.';
COMMENT ON COLUMN ASOP_CARD_MIFARES.MEMORY_MAP IS 'Карта приложений и секторов чипа. Хранит структуру записанных тарифов.';
...
COMMENT ON TABLE ASOP_CARD_BANKS IS 'Данные банковских карт. PAN хранится только в виде токена/криптотекста (соответствие PCI DSS).';
COMMENT ON COLUMN ASOP_CARD_BANKS.PAN_TOKEN IS 'Токенизированный номер карты. Используется для проведения транзакций через эквайринг.';
COMMENT ON COLUMN ASOP_CARD_BANKS.PAN_LAST4 IS 'Последние 4 цифры номера. Разрешено к отображению в чеках и интерфейсе.';
COMMENT ON COLUMN ASOP_CARD_BANKS.BIN IS 'Банковский идентификационный номер. Определяет эквайринг-партнёра и тип карты.';
COMMENT ON COLUMN ASOP_CARD_BANKS.IS_TOKENIZED IS 'Признак цифрового кошелька (Apple/Google/Mir Pay). Требует проверки криптограммы при оплате.';
...
COMMENT ON TABLE ASOP_CARD_TARIFFS IS 'Активные тарифные продукты, записанные на карту. Одна карта может содержать несколько продуктов одновременно.';
COMMENT ON COLUMN ASOP_CARD_TARIFFS.BALANCE IS 'Остаток средств (для кошелька).'
COMMENT ON COLUMN ASOP_CARD_TARIFFS.TRAVEL_COUNT IS 'Остаток поездок (для пакетов/безлимита).';
COMMENT ON COLUMN ASOP_CARD_TARIFFS.PURCHASE_TRANSACTION_ID IS 'Ссылка на транзакцию оплаты/пополнения данного тарифа.';
...
COMMENT ON TABLE ASOP_BLACKLISTS IS 'Реестр заблокированных карт. Терминалы получают этот список для мгновенного запрета прохода.';
COMMENT ON COLUMN ASOP_BLACKLISTS.BLOCK_TYPE IS 'Причина блокировки: PERMANENT (утеря/кража/мошенничество) или NEGATIVE_BALANCE (технический минус).';
...
COMMENT ON TABLE ASOP_BENEFITS IS 'Справочник государственных субсидий, загружаемый из реестра Минтруда/Соцзащиты.';
COMMENT ON COLUMN ASOP_BENEFITS.REGION_CODE IS 'Код региона действия льготы (например, субъект РФ или муниципалитет).';
...
COMMENT ON TABLE ASOP_BENEFIT_STEPS IS 'Ступенчатая модель скидок. Определяет, какая доля стоимости оплачивается государством на определённом количестве поездок в месяц.';
COMMENT ON COLUMN ASOP_BENEFIT_STEPS.TRIP_THRESHOLD_FROM IS 'Нижняя граница диапазона поездок (включительно).';
COMMENT ON COLUMN ASOP_BENEFIT_STEPS.TRIP_THRESHOLD_TO IS 'Верхняя граница. NULL означает "до конца периода".';
COMMENT ON COLUMN ASOP_BENEFIT_STEPS.DISCOUNT_SHARE IS 'Доля скидки: 0.0 = бесплатно, 0.5 = скидка 50%, 1.0 = полная оплата.';
...
COMMENT ON TABLE ASOP_USER_BENEFITS IS 'Активные льготы, закреплённые за профилем пассажира. Назначаются вручную после предоставления СНИЛС.';
COMMENT ON COLUMN ASOP_USER_BENEFITS.VALID_FROM IS 'Дата начала действия социальной программы.';
COMMENT ON COLUMN ASOP_USER_BENEFITS.VALID_UNTIL IS 'Дата окончания. NULL означает бессрочное действие до отзыва.';
COMMENT ON COLUMN ASOP_USER_BENEFITS.SYNC_VERSION IS 'Версия записи. Увеличивается при любом изменении для отслеживания устаревших данных на терминалах.';
...
COMMENT ON TABLE ASOP_ROUTE_BENEFITS IS 'Белый список льгот для маршрутов. Используется только если у маршрута BENEFIT_POLICY = ''ALLOWLIST''.';
COMMENT ON COLUMN ASOP_ROUTE_BENEFITS.BENEFIT_ID IS 'Конкретная льгота, разрешённая к применению на данном маршруте.';
...
COMMENT ON TABLE ASOP_USER_SESSIONS IS 'Рабочие смены терминалов. Фиксирует время работы, ответственных лиц и привязку к карте (если требуется).';
COMMENT ON COLUMN ASOP_USER_SESSIONS.OPENED_BY_USER_ID IS 'Сотрудник, открывший смену. Может отличаться от закрывшего.';
COMMENT ON COLUMN ASOP_USER_SESSIONS.STARTED_AT_LOCAL IS 'Время открытия по локальным часам терминала (для сверки при рассинхроне NTP).';
...
COMMENT ON TABLE ASOP_EVENTS IS 'Журнал всех системных действий. Используется для аудита, разбора инцидентов и отчётности.';
COMMENT ON COLUMN ASOP_EVENTS.REFERENCE_ID IS 'Идентификатор связанной сущности (карты, транзакции, тарифа). Тип определяется в REFERENCE_TYPE_ID.';
...
COMMENT ON TABLE ASOP_TRANSACTIONS IS 'Финансовые и тарифные проводки. Хранятся ВСЕ попытки оплаты: успешные, отклонённые, ожидающие.';
COMMENT ON COLUMN ASOP_TRANSACTIONS.STARTED_AT IS 'Момент начала обработки операции (прикладывание карты/запрос к эквайрингу).';
COMMENT ON COLUMN ASOP_TRANSACTIONS.COMPLETED_AT IS 'Момент финализации. NULL означает процесс валидации или ожидание ответа банка.';
COMMENT ON COLUMN ASOP_TRANSACTIONS.AMOUNT IS 'Итоговая сумма проводки. Учитывает применённые скидки и ступени льгот.';
COMMENT ON COLUMN ASOP_TRANSACTIONS.ACQUIRER_REFERENCE IS 'Уникальный номер операции в платёжной системе банка. Нужен для сверки и возвратов.';
COMMENT ON COLUMN ASOP_TRANSACTIONS.ERROR_CODE IS 'Код ошибки от терминала, банка или расчётного ядра.';
...
COMMENT ON TABLE ASOP_TRANSACTION_CARDS is 'Связка карт с транзакцией. Позволяет фиксировать несколько носителей в одной операции.';
COMMENT ON COLUMN ASOP_TRANSACTION_CARDS.CARD_ROLE IS 'Роль носителя: PAYER (оплата), IDENTIFIER (идентификация/льгота), FALLBACK (резервная карта).';
COMMENT ON COLUMN ASOP_TRANSACTION_CARDS.BALANCE_BEFORE IS 'Баланс карты до операции. Фиксируется для аудита и отмены транзакций.';
...
COMMENT ON TABLE ASOP_CARRIER_ROUTES IS 'Договорные связи: какие маршруты эксплуатируются конкретным перевозчиком.';
...
COMMENT ON TABLE ASOP_TRIPS IS 'Фактические рейсы транспортных средств. Связывает маршрут, смену водителя и временные интервалы.';
COMMENT ON COLUMN ASOP_TRIPS.CLOSED_AT IS 'Время прибытия на конечную/закрытия рейса. NULL означает рейс в пути.';
...
-- 8. ДОКУМЕНТАЦИЯ
-- ========================
COMMENT ON TABLE ASOP_EVENTS IS 'Журнал системных действий. EVENT_OBJECT хранит детализированный контекст в JSON.';
COMMENT ON TABLE ASOP_TRANSACTIONS IS 'Реестр финансовых операций. Служит источником данных для биллинга, отчётности и сверки с банками.';
COMMENT ON TABLE ASOP_TRANSACTION_CARDS IS 'Участники транзакции. Фиксирует баланс и роль каждого носителя.';
COMMENT ON TABLE ASOP_CARD_MIFARES IS 'Технические профили NFC-карт.';
COMMENT ON TABLE ASOP_CARD_BANKS IS 'Профили банковских карт. PAN токенизирован.';
COMMENT ON TABLE ASOP_USER_SESSIONS IS 'Учёт рабочего времени терминалов и персонала.';
COMMENT ON TABLE ASOP_TARIFF_RATES IS 'Актуальные тарифы. Иерархия: Маршрут → Зона → Перевозчик → Система.';
COMMENT ON TABLE ASOP_FARE_ZONES IS 'Географические тарифные пояса.';
COMMENT ON TABLE ASOP_TRANSPORT_STOPS IS 'Остановочные пункты.';
COMMENT ON TABLE ASOP_ROUTE_TRANSPORT_STOPS IS 'Порядок следования остановок.';
COMMENT ON TABLE ASOP_TRANSPORT_STOP_ZONES IS 'Карта принадлежности остановок к зонам.';
COMMENT ON TABLE ASOP_BLACKLISTS IS 'Список недействительных карт.';
COMMENT ON TABLE ASOP_ROUTES IS 'Маршрутная сеть. BENEFIT_POLICY управляет доступностью субсидий.';
COMMENT ON TABLE ASOP_BENEFITS IS 'Справочник государственных программ поддержки.';
COMMENT ON TABLE ASOP_BENEFIT_STEPS IS 'Параметры ступенчатого расчёта скидок.';
COMMENT ON TABLE ASOP_USER_BENEFITS IS 'Персональные льготы пассажиров.';
COMMENT ON TABLE ASOP_ROUTE_BENEFITS IS 'Разрешённые льготы для маршрутов с политикой ALLOWLIST.';